...
public class QuestionSixActivity extends ActivityMaster {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.questionsix);

        // retrieve from previous activity
        int previousScore = YourPreference.getInstance(this).getIntData("QUESTION_FIVE", 0);

        // clears previous SharedPreferences data
        YourPreference.getInstance(this).removeData("QUESTION_SIX");
        ...
        option1Button.setOnClickListener(v -> {
            onAnswerSelected("Magong");
        });


        option2Button.setOnClickListener(v -> {
            onAnswerSelected("Tainan City");
        });


        option3Button.setOnClickListener(v -> {
            onAnswerSelected("Taipei");
        });
    ...
    protected void incrementScore(String selectedAnswer) {
        ...
        int newScore = currentScore + questionFiveScore;
    ...
    protected String[] getQuestionData(int questionIndex) {
        ...
        return new String[]{"Question Six", "Option A", "Option B", "Option C"};
    }

    @Override
    protected int getLayoutResourceId() {
        //shows matching xml file
        return R.layout.questionsix;
    }
